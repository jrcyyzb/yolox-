...
class rotate_BboxOverlaps2D:
    ...
    def __call__(self, bboxes1, bboxes2, mode='iou', is_aligned=False):
        ...
        if self.dtype == 'fp16':
            ...
            overlaps = rotatebbox_overlaps(bboxes1, bboxes2, mode, is_aligned)
            if not overlaps.is_cuda and overlaps.dtype == torch.float16:
                # resume cpu float32
                overlaps = overlaps.float()
            return overlaps

        return rotatebbox_overlaps(bboxes1, bboxes2, mode, is_aligned)
...
def rotatebbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6):
