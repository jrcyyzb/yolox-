...
class rotate_YOLOXHead(BaseDenseHead, BBoxTestMixin):
    ...
    def _build_predictor(self):
        ...
        conv_reg = nn.Conv2d(self.feat_channels, 5, 1)
        conv_obj = nn.Conv2d(self.feat_channels, 1, 1)
        return conv_cls, conv_reg, conv_obj

    def init_weights(self):
        super(rotate_YOLOXHead, self).init_weights()
    ...
    def _bbox_decode(self, priors, bbox_preds): # I transfome it to rotate,predicted is Affine Matrix parameter 这里还可以改进
        xys = (bbox_preds[..., :2] * priors[:, 2:]) + priors[:, :2] # priors=[[coord_x, coord_y, stride_w, stride_h],...]
